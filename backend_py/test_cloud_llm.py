...
class LLMClient:
    ...
    def __init__(
        ...
        if self.llm_type == "cloud":
            ...
            try:
                ...
                print(f"鉁?Using Cloud LLM: {self.host}")
            except ImportError:
            ...
            self.client = None
            print(f"鉁?Using Local Ollama: {self.host}")
    ...
    def _generate_cloud(self, prompt: str, system: str = "") -> str:
        ...
        try:
            ...
            # 馃敡 A more conservative token estimate (to avoid exceeding limits)
            # We use a coefficient of 2.0 instead of 1.3 because the actual number of tokens is often much larger.
            input_tokens_estimate = len(prompt.split()) * 2.0

            # Calculate available space (reserve a 300-token safety buffer).
            available_space = 4096 - int(input_tokens_estimate) - 300

            # Set max_tokens: minimum 200, maximum 1200, not exceeding available space.
            max_tokens = max(200, min(1200, available_space))

            # If the available space is too small, use the minimum value.
            if available_space < 200:
                max_tokens = 200
                print("鈿狅笍  Warning: Input is very large, output will be limited")

            # Debug logs
            print(f"馃搳 Token estimation:")
            print(f"   - Input estimate: ~{int(input_tokens_estimate)} tokens")
            print(f"   - Available space: ~{available_space} tokens")
            print(f"   - Using max_tokens: {max_tokens}")
            print(
                f"   - Total Expected: ~{int(input_tokens_estimate) + max_tokens} / 4096"
            )
...
def robust_loads(maybe_json: str) -> Dict[str, Any]:
    ...
    """
    # Debugging: Print the raw response (first 1000 characters)
    print("=" * 80)
    print("馃攳 DEBUG: Raw LLM Response")
    print("=" * 80)
    ...
    try:
        ...
        print("鉁?JSON parsed successfully (direct)")
        return obj
    except Exception as e:
        print(f"鈿狅笍  Direct JSON parse failed: {e}")
        print("馃攧 Trying cleanup...")

    s = maybe_json.strip()

    # Clean up markdown code blocks
    if "```" in s:
        print("馃Ч Removing markdown code fences...")
    ...
    if l != -1 and r != -1 and r > l:
        s2 = s[l : r + 1]
        print(f"馃Ч Extracted JSON from position {l} to {r}")
        print("馃攳 Cleaned content:")
        print(s2[:500])
        print("=" * 80)
        try:
            obj = json.loads(s2)
            if not isinstance(obj, dict):
                raise ValueError("Top-level JSON must be an object")
            print("鉁?JSON parsed successfully (after cleanup)")
            return obj
        except Exception as e:
            print(f"鉂?Cleaned JSON parse failed: {e}")
            print(f"馃摑 Failed content (first 500 chars): {s2[:500]}")

    print("=" * 80)
    print("鉂?All parsing attempts failed")
    print(f"馃摑 Original content (first 1000 chars):")
    print(maybe_json[:1000])
    print("=" * 80)
