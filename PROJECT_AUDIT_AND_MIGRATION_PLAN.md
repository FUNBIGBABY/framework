...
5. Phase 1：Argon2id、JWT 单一信任源、白名单注册。
6. Phase 2：抽 `LLMProvider` / `VectorStoreProvider` / `EmbeddingProvider` / `ObjectStoreProvider`。
7. Phase 3：DeepSeek V4 接入，JSON Output 只用于结构化输出，thinking 默认显式控制。
8. Phase 4：Postgres + pgvector + Alembic，一次建好 Agent / RAG / LLMWiki 占位表。
9. Phase 5-7：后端接管 Firestore 业务，前端去 Firebase，域名和客户遗留清理。
10. Phase 8：Agent 核心循环 + Tool Registry + SSE + trace。
11. Phase 9：RAG 证据检索层，上传、chunk、embedding、citation。
12. Phase 10：LLMWiki 知识编译层，wiki pages / claims / links / eval questions。
13. Phase 11：Skill Registry + Chat UI + Agent Eval Harness。
14. Phase 12：部署运维 + read-only MCP-compatible adapter。
15. Phase 13：个人自用合规边界、ICP、出网 allowlist、数据删除能力。
