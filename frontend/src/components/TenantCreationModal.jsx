...
/**
 * TenantCreationModal - A modal that forces the creation of a Tenant.
 *
 * Features:
 * - A tenant must be created upon first login.
 * - Cannot be closed (forced creation)
...
function TenantCreationModal({ onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    displayName: 'AI Readiness Expert', // default value
    subdomain: 'ai-readiness', // default value
    allowedOrigins: 'https://valorie.ai', // default value
  })
  ...
  // Handling subdomain input changes
  const handleSubdomainChange = e => {
    const value = e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '')
    setFormData({ ...formData, subdomain: value })

    // Delayed check availability
    if (value.length >= 3) {
  ...
  const handleCreateTenant = async e => {
    ...
    try {
      ...
      // Double-check if the subdomain exists (to prevent concurrent creation).
      const tenantDoc = await getDoc(doc(db, 'tenants', tenantId))
      if (tenantDoc.exists()) {
        throw new Error('This subdomain is already taken')
      }

      // 1. Creating a Tenant document
        ...
        members: [
          {
            userId: user.uid,
            role: 'owner',
            joinedAt: new Date().toISOString(), // ✅ Use the ISO string instead of serverTimestamp
          },
        ],
        embedKey: embedKey,
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full shadow-2xl">
        ...
        <div className="px-8 py-4 bg-blue-50 border-l-4 border-blue-500">
          <p className="text-sm text-blue-800">
            <strong>What is a tenant?</strong> Your tenant is your expert
            workspace. It allows you to:
          </p>
        ...
        <div className="px-8 py-4 bg-gray-50 border-t border-gray-200 rounded-b-lg">
          <p className="text-xs text-gray-600">
            <strong>Note:</strong> You can change these settings later in Tenant
            Settings.
          </p>
        </div>
      </div>
