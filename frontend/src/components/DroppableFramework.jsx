...
export default function DroppableFramework({
  ...
  // Create a droppable for the entire framework (but only for the Description).
  const { setNodeRef: setFrameworkRef, isOver: isFrameworkOver } = useDroppable(
    {
      ...
      data: {
        ...
        acceptTypes: ['description'], // Only accept Description
      },
    }
  )

  // Create separate droppables for sub-steps regions.
  const { setNodeRef: setSubStepsRef, isOver: isSubStepsOver } = useDroppable({
    id: `substeps-${framework.id}`,
    data: {
      type: 'substeps-area',
      frameworkId: framework.id,
      acceptTypes: ['substep'], // Only accepts Sub-steps
    },
  })

  // Create a unique list of IDs for sub-steps
  const subStepIds = framework.subSteps
    ? framework.subSteps.map((_, idx) => `substep-${framework.id}-${idx}`)
    : []

  // Determine if dragging is currently in progress.
  const isDraggingDescription =
    activeId && activeId.toString().startsWith('desc-')

  // Determine if a substep is currently being dragged.
  ...
  return (
    ...
    >
      ...
      {/* Description Area - Individual droppable */}
      <div
        ref={setFrameworkRef}
      ...
      {/* Sub-steps Area - Remove external highlights */}
      {framework.subSteps && framework.subSteps.length > 0 ? (
        <div ref={setSubStepsRef}>
          ...
          >
            {/* ✅ The prompt will only be displayed when the page is empty.*/}
            {isDraggingSubstep &&
              isSubStepsOver &&
