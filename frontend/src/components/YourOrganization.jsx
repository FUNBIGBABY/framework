...
/**
 * YourOrganization - Organization Page (Fixed Version)
 *
 * ✅ Core fixes:
 * - When a user accesses /:tenantId/organization, the content displayed is for joinedOrganization.
 * - For example: User C accesses /ai-readiness4/organization, but the data for ai-readiness2 is displayed.
 */
function YourOrganization() {
  const navigate = useNavigate()
  const { tenantId } = useParams() // The tenantId in the URL (user's own).
  ...
  // ✅ Fix: Determine the organization ID to query
  // - If a user has joined an organization, display the content of that organization.
  // - If the user is the owner (not joined to any other organization), display their own organization's content.
  ...
  const filteredMyFrameworks =
    selectedMember === 'all' || selectedMember === user?.uid ? myFrameworks : []
  ...
  // 在第51行之前（useEffect之前）添加
  const loadOrganizationData = useCallback(async () => {
    try {
      setLoading(true)

      // ✅ Fix: Display an empty status if the user has not joined any organization.
      ...
      // ✅ Fix: Query the correct organization data
      ...
      if (organizationToQuery === user?.tenantId) {
        ...
        // If it's an organization you've joined
      ...
      // ✅ Fix: Loads the correct organization's member list
      const membersData = await getTenantMembers(organizationToQuery)
      console.log('✅ Loaded members:', membersData.length)
      setMembers(membersData)

      // ✅ Fix: Loads the correctly organized shared framework
      const frameworksData =
        await getOrganizationFrameworks(organizationToQuery)
  ...
  }, [organizationToQuery, user?.tenantId, user?.joinedOrganization, tenantId])

  useEffect(() => {
    if (!user) return

    // ✅ Fix: Only check if the tenantId in the URL belongs to the user.
  ...
  }, [tenantId, user, loadOrganizationData, navigate])

  const handleLeaveOrganization = async () => {
    // ✅ Fix: Only members can leave the organization.
    if (organizationToQuery === user.tenantId) {
    ...
    try {
      ...
      // Refresh the page to show an empty state.
  ...
  // ✅ Fix: Display a prompt if the user is not in any organization.
  if (!organizationToQuery) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">
            You haven't joined any organization yet. Ask a team leader for an
            invitation link.
          </p>
          <button
      ...
      <div className="max-w-7xl mx-auto py-8 px-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              {/* ✅ Added: Display organization ID information */}
              <p className="text-xs text-gray-400 mt-1">
                Organization ID: {organizationToQuery}
              </p>
            </div>

            {/* Leave Organization Button (visible only to members) */}
            {organizationToQuery !== user.tenantId && (
              <button
          ...
          <div className="flex items-center justify-between">
            ...
            {/* ✅ Add a "+" invite button - only the owner can see it. */}
            {organizationToQuery === user.tenantId && (
              <button
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6 flex items-center justify-between">
          ...
          <span className="text-sm text-gray-500">
            {filteredMyFrameworks.length + filteredTeamFrameworks.length}{' '}
            frameworks
          </span>
        </div>
          ...
          filteredTeamFrameworks.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                No shared frameworks yet. Publish your frameworks to share with
                the team!
              </p>
            </div>
          )}
...
/**
 * FrameworkCard - Frame Card Component
 */
function FrameworkCard({ framework, isOwner, onRefresh, members = [] }) {
  ...
  const handleUnpublish = async () => {
    ...
    const confirmed = confirm(
      'Are you sure you want to unpublish this framework?'
    )
    if (!confirmed) return
  ...
  return (
    <>
      <div className="bg-white border border-gray-200 rounded-lg p-5 hover:shadow-lg transition-shadow">
        ...
        {framework.category && (
          <span
            className={`inline-block px-3 py-1 text-xs rounded-full mb-3 font-medium ${getCategoryColor(framework.category)}`}
          >
            {framework.category}
          </span>
        )}
        ...
        <div className="text-xs text-gray-500 mb-4">
          Updated:{' '}
          {framework.updatedAt?.toDate?.()?.toLocaleDateString() || 'Unknown'}
        </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  ...
                  <div>
                    Family: {framework.family || framework.category || 'N/A'}
                  </div>
