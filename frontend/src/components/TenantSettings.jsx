...
function TenantSettings() {
  ...
  const [inviteEmails, setInviteEmails] = useState(['']) // Change to array

  // Invitation link status
  const [showInviteSection, setShowInviteSection] = useState(false)
  const [inviteMaxUses, setInviteMaxUses] = useState(1) // 1 or -1
  ...
  const loadTenant = useCallback(async () => {
    try {
      ...
      // ✅ Fix: Correctly loads organization data in useEffect.
  ...
  // ✅ Fix: Determine if the user is the owner
  const isOwner = tenant?.members?.some(
  ...
  // When maxUses changes, reset the email array.
  useEffect(() => {
    if (inviteMaxUses === 1 && inviteEmails.length > 1) {
      // When switching from Unlimited to Single Use, only the first email is retained.
      setInviteEmails([inviteEmails[0] || ''])
    }
  }, [inviteMaxUses, inviteEmails])

  // ✅ Fix: Correctly implements handleLeaveOrganization
  const handleLeaveOrganization = async () => {
    ...
    try {
      ...
      // Clear organization-related status
  ...
  const handleGenerateInviteLink = async () => {
    // ✅ Verify email format - Change to verify all emails
    ...
    try {
      ...
      // Generate an invitation link for each email
      const results = []
      ...
      setGeneratedInviteLink(results[0]) // Show the first generated link
      await loadTenant()

      // ✅ Reset the email array upon success
      setInviteEmails([''])
  ...
  const copyToClipboard = text => {
    // Try using the modern Clipboard API (HTTPS required).
    if (navigator.clipboard && navigator.clipboard.writeText) {
      ...
      // Use fallback directly (HTTP environment)
      fallbackCopyToClipboard(text)
    }
  }
            ...
            {isOwner && showInviteSection && (
              <div className="border-t border-gray-200 pt-6">
                ...
                <div className="space-y-4 mb-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e =>
                          setInviteExpiresInDays(Number(e.target.value))
                        }
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600"
                      >
                  ...
                  {/* ✅ New Feature: Email Input Field - Displays one or more emails based on maxUses */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Invitee Email{inviteMaxUses === -1 ? 's' : ''}{' '}
                      <span className="text-red-500">*</span>
                    </label>

                    <div className="space-y-3">
                      {inviteMaxUses === -1 ? (
                        // Unlimited: Displays multiple email input boxes.
                        <>
                          ...
                          <p className="text-xs text-gray-500">
                            Multiple invitations will be generated, one for each
                            email address
                          </p>
                        </>
                      ) : (
                        // Single Use: Displays only one email input field.
                        <>
                          <input
                            type="email"
                          ...
                          />
                          <p className="text-xs text-gray-500">
                            Only this email address will be able to accept the
                            invitation
                          </p>
                        </>
                      )}
                ...
                {generatedInviteLink && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() =>
                          copyToClipboard(generatedInviteLink.inviteLink)
                        }
                    ...
                    <p className="text-xs text-green-700 mt-2">
                      Expires: {formatDate(generatedInviteLink.expiresAt)} • Max
                      uses:{' '}
                      {generatedInviteLink.maxUses === -1
                        ? 'Unlimited'
                        : generatedInviteLink.maxUses}
                    </p>
                  </div>
                )}
                ...
                {/* Active Invite Links */}
                {tenant.inviteLinks &&
                  tenant.inviteLinks.filter(link => link.isActive).length >
                    0 && (
                    <div className="mt-6">
                          ...
                          .map(link => (
                            ...
                            >
                              <div className="text-sm">
                                ...
                                <p className="text-xs text-gray-500">
                                  Used: {link.usedCount}/
                                  {link.maxUses === -1 ? '∞' : link.maxUses} •
                                  Expires: {formatDate(link.expiresAt)}
                                </p>
                              </div>
                              <button
                                onClick={() =>
                                  handleRevokeInviteLink(link.token)
                                }
            ...
            <div className="space-y-4 mb-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900 mt-1">
                    {joinedOrganization.displayName}
                  </p>
                </div>
              ...
              <div>
                ...
                <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Usage:</strong> Paste this code into your microsite
                    HTML (e.g., valorie.ai/ai-readiness). The button will open a
                    modal that shows only your published frameworks.
                  </p>
                </div>
              </div>
            )}
