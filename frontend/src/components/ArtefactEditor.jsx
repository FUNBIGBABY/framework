...
const convertVariantToHTML = variant => {
...
const parseHTMLToVariant = (html, originalVariant) => {
  ...
  if (table) {
    ...
    const sections = rows.map((row, index) => {
      ...
      const headingText =
        cells[0]?.textContent?.trim() || `Section ${index + 1}`
...
const ArtefactEditor = ({
  ...
  // Update editor content when variant changes externally
  useEffect(() => {
    if (editor && variant) {
  ...
  const handleDelete = () => {
    if (
      window.confirm(
        `Are you sure you want to delete "${variant?.name || 'this artefact'}"?`
      )
    ) {
      onDelete(index)
    }
  ...
  const handleAIFill = async () => {
    ...
    const shortSections = sectionsToFill.filter(
      s => s.heading.length < 5 || /^\d+$/.test(s.heading)
    )
    ...
    try {
      ...
      if (result.success && result.filled_sections) {
        ...
        alert(
          `✨ AI filled ${result.filled_sections.length} section(s) successfully!`
        )
      } else {
